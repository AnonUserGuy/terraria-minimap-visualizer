...
export class SchematicWriter {
        ...
        for (let x = 0; x < worldMap.width; x++) {
            for (let y = 0; y < worldMap.height; y++) {
                ...
                if (cell !== worldMap.mapData.unexploredTile) {
                    if (frameImportant) {
                        this.getTileUVFromGeometry(worldMap, cell, x, y, u, v);
                        const tileData = worldMap.mapData.tile(cell.id);
                        if (tileData.type) {
                            [u2, v2] = this.getTileUVFromOption(tileData.type, cell.option);
    ...
    static serializeCellData(cell, u, v, needsWall, wall, frameImportant) {
    ...
    static getTileUVFromGeometry(worldMap, tile, x, y, u, v) {
    ...
    static getTileUVFromOption(optionType, option) {
        const result = [0, 0];
        switch (optionType) {
    ...
    static resolveWidth(worldMap, x, y, width, u) {
        const i = y * worldMap.width + x;
    ...
    static resolveHeight(worldMap, x, y, height, v) {
        const i = y * worldMap.width + x;
        const tile = worldMap.cells[i];
        if (y === 0) {
            v[i] = 0;
    ...
    static resolveTree(worldMap, x, y, u, v) {
    ...
    static plantDetritus3(worldMap, x, y, u, v) {
        let i = y * worldMap.width + x;
    ...
    static plantDetritus2(worldMap, x, y, u, v) {
        let i = y * worldMap.width + x;
        let i2 = (y + 1) * worldMap.width + x;
        u[i] = 0;
        v[i] = 36;
